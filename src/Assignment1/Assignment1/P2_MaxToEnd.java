...
public class P2_MaxToEnd
{
  public static void main(String[] args)
  {
    // The statement "Integer[] ..." creates an array obj which is used to initialize ArrayList obj.
  ...
  {
    int greatest = 0; //variable to hold the biggest element
    int i = 0, position = 0; //iterator and element position variable
    for (int numbers : nums)
    {
      if(numbers > greatest)
      {
        greatest = numbers; // set the biggest number seen so far
        position = i; // set the position in the array for the greatest number seen so far
      }
